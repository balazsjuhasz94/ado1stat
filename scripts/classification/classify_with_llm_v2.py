"""
Classify organizations using LLM (Claude Haiku) - VERSION 2
- Uses categories_with_descriptions.yaml with detailed guidance
- Includes both organization purpose AND headquarters location
- Works with organizations scraped by tax number (adoszam)
"""
...
def main():
    ...
    print("="*70)
    print("SZERVEZETEK KATEGORIZÁLÁSA LLM-MEL V2 (Claude Haiku)")
    print("Új kategóriák leírásokkal + Székhely információval")
    print("="*70)
    ...
    if not api_key:
        ...
        print("Mentsd az API kulcsot api.txt fájlba vagy állítsd be környezeti változóként.")
        return
    ...
    categories = load_categories_with_descriptions('categories_with_descriptions.yaml')
    print(f"   {len(categories)} kategória betöltve leírásokkal")

    # Format categories for prompt
    categories_text = format_categories_for_prompt(categories)

    # Load target tax numbers from adoszam_500.txt
    print("\n2. Célzott adószámok betöltése (adoszam_500.txt)...")
    target_adoszamok = set()

    if os.path.exists('adoszam_500.txt'):
        with open('adoszam_500.txt', 'r', encoding='utf-8') as f:
            for line in f:
                adoszam = line.strip()
                if adoszam:
                    target_adoszamok.add(adoszam)
        print(f"   {len(target_adoszamok)} adószám betöltve")
    else:
        print("   FIGYELEM: adoszam_500.txt nem található, az összes szervezetet feldolgozom")

    # Load organizations from organizations_by_adoszam folder
    print("\n3. Szervezetek betöltése (organizations_by_adoszam)...")
    ...
    if not os.path.exists(organizations_folder):
        ...
        print(f"   Először futtasd: python run_scraper_adoszam.py --input adoszam_500.txt")
        return
    ...
    for filename in json_files:
        ...
        try:
            ...
            # Filter: only include if adoszam is in target list (or if no filter file exists)
            if target_adoszamok and adoszam not in target_adoszamok:
                skipped += 1
                continue
    ...
    if skipped > 0:
        print(f"   {skipped} szervezet kihagyva (nincs az adoszam_500.txt-ben)")
    ...
    # Classify in batches
    print("\n4. Klasszifikálás (batch-enként, Claude Haiku)...")
    ...
    # Save to CSV
    output_file = 'organization_categories_adoszam_500.csv'
    df_results.to_csv(output_file, index=False, encoding='utf-8-sig')

    print(f"\n7. Eredmények mentve: {output_file}")
    print("="*70)
    print("\nKÉSZ!")
